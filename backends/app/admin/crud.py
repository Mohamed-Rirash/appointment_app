...
from app.auth.models import roles, permissions, user_roles, role_permissions
from app.admin.models import (
    admin_audit_logs, admin_sessions, system_config, data_exports,
    system_metrics, admin_notifications, user_activities, system_alerts,
    admin_preferences, bulk_operations, system_backups
)
from app.admin.schemas import (
    AdminActionType, PaginationParams, UserSearchFilters, 
    AdminActivityFilters, SortOrder
)
from app.admin.exceptions import UserNotFoundError, RoleNotFoundError
...
class AdminUserCRUD:
    ...
    @staticmethod
    async def get_user_with_roles(db: Database, user_id: UUID) -> Optional[Dict[str, Any]]:
        ...
        roles_query = select(
            ...
            roles.join(user_roles, roles.c.id == user_roles.c.role_id)
        ...
        permissions_query = select(
            ...
            permissions
            .join(role_permissions, permissions.c.id == role_permissions.c.permission_id)
            .join(user_roles, role_permissions.c.role_id == user_roles.c.role_id)
        ).where(user_roles.c.user_id == user_id).distinct()
...
class AdminRoleCRUD:
    ...
    @staticmethod
    async def get_role_with_permissions(db: Database, role_id: UUID) -> Optional[Dict[str, Any]]:
        ...
        permissions_query = select(
            permissions.c.id,
            permissions.c.name,
            permissions.c.display_name
        ).select_from(
            permissions.join(role_permissions, permissions.c.id == role_permissions.c.permission_id)
    ...
    async def get_audit_logs_paginated(
        ...
        if filters:
            ...
            if filters.resource_type:
                conditions.append(admin_audit_logs.c.resource_type == filters.resource_type)
...
class AdminSystemCRUD:
    ...
    async def get_system_stats(db: Database) -> Dict[str, Any]:
        ...
        system_users = await db.fetch_val(
            select(func.count()).select_from(users).where(users.c.is_system_user == True)
        )
        ...
        total_permissions = await db.fetch_val(select(func.count()).select_from(permissions))
        active_permissions = await db.fetch_val(
            select(func.count()).select_from(permissions).where(permissions.c.is_active == True)
        )
        
        # Recent activity (last 24 hours)
        yesterday = datetime.now(timezone.utc) - timedelta(days=1)
        recent_logins = await db.fetch_val(
            select(func.count()).select_from(user_activities)
            .where(
                and_(
