...
from app.items.schemas import (
    ItemCreate, ItemUpdate, ItemFilters, PaginationParams,
    BulkItemOperation, BulkOperationResult, ItemStatusUpdate,
    CommentCreate, CommentUpdate, ItemPermissionCreate
)
from app.items.models import ItemStatus, items, item_comments, item_favorites, item_permissions
from app.items.dependencies import validate_item_status_transition
...
class ItemService:
    ...
    async def create_item(
        ...
        if not owner.is_verified:
            raise HTTPException(
                ...
                detail="Only verified users can create items"
            )
        
        # Business rule: Published items require admin approval for new users
        if (item_data.status == ItemStatus.PUBLISHED and 
            not owner.is_admin and 
            await ItemService._is_new_user(db, owner.id)):
            
            # Auto-change to draft for new users
            item_data.status = ItemStatus.DRAFT
        
        # Create the item
        item = await ItemCRUD.create(db, item_data, owner.id)
        
        # Log item creation
        await ItemService._log_item_activity(
    ...
    async def update_item(
        ...
        if updated_item:
            # Log item update
            changes = {k: v for k, v in item_data.model_dump(exclude_unset=True).items() if v is not None}
            await ItemService._log_item_activity(
    ...
    async def delete_item(
        ...
        # Business rule: Published items with comments require admin approval to delete
        if (current_item["status"] == ItemStatus.PUBLISHED and 
            current_item["comment_count"] > 0 and 
            not current_user.is_admin):
            
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Published items with comments can only be deleted by admins"
            )
        ...
        if success:
            # Log item deletion
            await ItemService._log_item_activity(
                db, item_id, current_user.id, "deleted", 
                {"soft_delete": soft_delete, "comment_count": current_item["comment_count"]}
            )
    ...
    async def get_item_list(
        ...
        items_list, total = await ItemCRUD.get_list(db, filters, pagination, current_user_id)
    ...
    async def bulk_operation(
        ...
        result = BulkOperationResult(
            total_items=len(operation_data.item_ids),
            successful_items=0,
            failed_items=0,
        ...
        for item_id in operation_data.item_ids:
            try:
                ...
                required_permission = "update" if operation_data.operation != "delete" else "delete"
                ...
                # Perform operation
                success = await ItemService._perform_bulk_operation(
        ...
        # Log bulk operation
        await ItemService._log_item_activity(
            db, None, current_user.id, f"bulk_{operation_data.operation}",
            {
    ...
    async def toggle_favorite(
        ...
        count_query = select(func.count()).select_from(item_favorites).where(
            item_favorites.c.item_id == item_id
        )
...
class ItemCommentService:
    ...
    async def get_item_comments(
        ...
        for result in results:
            ...
            comment_dict["can_update"] = (
                current_user_id == comment_dict["author_id"]
            )
            comment_dict["can_delete"] = (
                current_user_id == comment_dict["author_id"]
            )
