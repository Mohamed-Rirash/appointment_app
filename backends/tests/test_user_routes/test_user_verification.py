...
@pytest.mark.asyncio
async def test_user_account_verification(client, inactive_user, test_session):
    token_context = inactive_user.get_context_string(USER_VERIFY_ACCOUNT)
    token = await hash_password(token_context)
    data = {
...
@pytest.mark.asyncio
async def test_user_link_doesnot_work_twice(client, inactive_user, test_session):
    token_context = inactive_user.get_context_string(USER_VERIFY_ACCOUNT)
    token = await hash_password(token_context)
    time.sleep(1)
    data = {
...
@pytest.mark.asyncio
async def test_user_invalid_token_does_not_work(client, inactive_user, test_session):
    data = {
        "email": inactive_user.email,
...
@pytest.mark.asyncio
async def test_user_invalid_email_does_not_work(client, inactive_user, test_session):
    token_context = inactive_user.get_context_string(USER_VERIFY_ACCOUNT)
    token = await hash_password(token_context)
    data = {
