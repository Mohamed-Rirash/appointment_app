...
from app.admin.service import AdminSystemService, AdminUserService
...
class TestAdminUserManagement:
    ...
    async def test_admin_create_user(
        self, db_session: Database, super_admin_user: dict
    ):
    ...
    async def test_admin_update_user(
        self, db_session: Database, test_user: dict, super_admin_user: dict
    ):
        """Test admin user update"""

        update_data = AdminUserUpdate(first_name="Updated", is_verified=True)
    ...
    async def test_bulk_user_operations(
        ...
        bulk_operation = BulkUserOperation(user_ids=user_ids, operation="activate")
    ...
    @pytest.mark.asyncio
    async def test_system_user_protection(
        self, db_session: Database, system_admin_user: dict, super_admin_user: dict
    ):
...
class TestAdminAPIEndpoints:
    ...
    async def test_admin_dashboard_endpoint(
        ...
        response = await client.get(
            "/api/v1/admin/dashboard", headers=super_admin_auth_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_admin_users_list_endpoint(
        self, client: AsyncClient, super_admin_auth_headers: dict
    ):
        """Test admin users list endpoint"""

        response = await client.get(
            "/api/v1/admin/users?page=1&size=10", headers=super_admin_auth_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_admin_create_user_endpoint(
        self, client: AsyncClient, super_admin_auth_headers: dict
    ):
        ...
        response = await client.post(
            "/api/v1/admin/users", json=user_data, headers=super_admin_auth_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_admin_bulk_operations_endpoint(
        self, client: AsyncClient, super_admin_auth_headers: dict, db_session: Database
    ):
        ...
        bulk_data = {"user_ids": user_ids, "operation": "activate"}

        response = await client.post(
            "/api/v1/admin/users/bulk", json=bulk_data, headers=super_admin_auth_headers
        )
        ...
        assert data["successful_appointments"] == 2
        assert data["failed_appointments"] == 0

    @pytest.mark.asyncio
    async def test_admin_system_info_endpoint(
        self, client: AsyncClient, super_admin_auth_headers: dict
    ):
        """Test admin system info endpoint"""

        response = await client.get(
            "/api/v1/admin/system/info", headers=super_admin_auth_headers
        )
    ...
    async def test_admin_audit_logs_endpoint(
        ...
        response = await client.get(
            "/api/v1/admin/audit/logs?page=1&size=10", headers=super_admin_auth_headers
        )
    ...
    async def test_admin_unauthorized_access(
        self, client: AsyncClient, user_auth_headers: dict
    ):
        """Test that regular users cannot access admin endpoints"""

        response = await client.get(
            "/api/v1/admin/dashboard", headers=user_auth_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_admin_permission_levels(
        ...
        response = await client.post(
            "/api/v1/admin/users", json=user_data, headers=admin_auth_headers
        )

        assert response.status_code == 403
        data = response.json()
        assert (
            "Super admin" in data["detail"]
            or "Insufficient permissions" in data["detail"]
        )
