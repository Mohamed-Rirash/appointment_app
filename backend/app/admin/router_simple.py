...
async def admin_delete_user(
    ...
    db: Database = Depends(get_db),
    admin: CurrentUser = Depends(
        require_permissions("users:delete", require_all=False)
    ),
):
...
async def admin_assign_role_by_name(
    ...
    db: Database = Depends(get_db),
    admin: CurrentUser = Depends(
        require_permissions("roles:assign", require_all=False)
    ),
):
...
async def admin_revoke_role_by_name(
    ...
    db: Database = Depends(get_db),
    admin: CurrentUser = Depends(
        require_permissions("roles:revoke", require_all=False)
    ),
):
    role = await RoleCRUD.get_by_name(db, role_name)
    if not role:
