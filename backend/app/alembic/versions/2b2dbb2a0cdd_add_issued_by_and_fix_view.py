...
down_revision: Union[str, Sequence[str], None] = "8097e9b9fb66"
...
def upgrade() -> None:
    ...
    has_issued_by = any(c["name"] == "issued_by" for c in inspector.get_columns("appointments"))
    ...
    fk_exists = any(
        ...
        set(fk['constrained_columns']) == {'issued_by'}
        for fk in fks
    )
    ...
    if has_issued_by:
        has_created_by = any(c["name"] == "created_by" for c in inspector.get_columns("appointments"))
        if has_created_by:
            conn.execute(sa.text("UPDATE appointments SET issued_by = created_by WHERE issued_by IS NULL"))

        # 4) As a fallback, where issued_by is still NULL, set it to host_id (better than leaving NULL)
        conn.execute(sa.text("UPDATE appointments SET issued_by = host_id WHERE issued_by IS NULL"))

        # 5) Only try to alter the column to NOT NULL if it's currently nullable
        column_info = next((c for c in inspector.get_columns("appointments") if c["name"] == "issued_by"), None)
        if column_info and column_info["nullable"]:
            op.alter_column("appointments", "issued_by", existing_type=sa.UUID(), nullable=False)
...
def downgrade() -> None:
    ...
    conn = op.get_bind()
    conn.execute(sa.text("UPDATE appointments SET created_by = issued_by WHERE created_by IS NULL"))

    # 4) Make created_by not null to match the original state
    op.alter_column("appointments", "created_by", existing_type=sa.UUID(), nullable=False)

    # 5) Drop issued_by and its FK
    op.drop_constraint("fk_appointments_issued_by_users", "appointments", type_="foreignkey")
