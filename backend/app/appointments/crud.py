...
class AppointmentCrud:
    ...
    #
    # @staticmethod
    # async def get_slot_by_start_time(db: Database, date, slot_time):
    #     """Find a slot by its start time and date"""
    #     query = select(time_slot).where(
    #         (time_slot.c.date == date) & (time_slot.c.slot_start == slot_time)
    #     )
    #     return await db.fetch_one(query)
    #
    @staticmethod
    ...
    #     query = select(appointment_details).where(
    ...
    @staticmethod
    async def get_appointment_row_by_id(db, appointment_id):
        """Fallback: fetch directly from base appointments table by primary key."""
        query = select(appointments).where(appointments.c.id == appointment_id)
        return await db.fetch_one(query)

    @staticmethod
    async def get_slot_by_start_time(db, date, slot_time):
        query = select(time_slot).where(
            (time_slot.c.date == date) & (time_slot.c.slot_start == slot_time)
        )
        return await db.fetch_one(query)
    ...
    @staticmethod
    async def mark_slot_booked(db, slot_id: str):
        query = (
            update(time_slot).where(time_slot.c.id == slot_id).values(is_booked=True)
        )
        await db.execute(query)
    ...
    @staticmethod
    async def free_slot(db, slot_id: str):
        query = (
            update(time_slot).where(time_slot.c.id == slot_id).values(is_booked=False)
        )
        await db.execute(query)
