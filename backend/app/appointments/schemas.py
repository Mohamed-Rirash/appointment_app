...
class ItemCreate(ItemBase):
    ...
    status: appointmentstatus = Field(
        appointmentstatus.DRAFT, description="Initial item status"
    )
...
class appointmentstatusUpdate(BaseModel):
    ...
    status: appointmentstatus = Field(..., description="New item status")
...
class ItemResponse(ItemBase):
    ...
    can_comment: bool = Field(
        False, description="Current user can comment on this item"
    )
    ...
    favorite_count: int = Field(
        0, description="Number of users who favorited this item"
    )
    is_favorited: bool = Field(
        False, description="Current user has favorited this item"
    )
...
class ItemListResponse(BaseModel):
    ...
    appointments: List[ItemResponse]
    total: int
    page: int
    size: int
    pages: int
...
class ItemPermissionCreate(ItemPermissionBase):
    ...
    user_id: Optional[UUID] = Field(
        None, description="User ID (for user-specific permissions)"
    )
    role_id: Optional[UUID] = Field(
        None, description="Role ID (for role-based permissions)"
    )
...
class appointmentsortOptions(str, Enum):
...
class PaginationParams(BaseModel):
    ...
    sort: appointmentsortOptions = Field(
...
class ItemViewCreate(BaseModel):
    ...
    view_duration: Optional[int] = Field(
        None, ge=0, description="View duration in seconds"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional view metadata"
    )
...
class BulkItemOperation(BaseModel):
    ...
    item_ids: List[UUID] = Field(
        ..., min_length=1, max_length=100, description="List of item IDs"
    )
    operation: str = Field(
        ...,
        pattern="^(delete|archive|publish|draft)$",
