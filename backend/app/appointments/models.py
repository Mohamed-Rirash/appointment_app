...
from sqlalchemy import (
    Table, Column, String, Boolean, DateTime, Text, Integer,
    ForeignKey, JSON, Float, Enum as SQLEnum, func
)
...
items = Table(
    ...
    Column("category", SQLEnum(ItemCategory), nullable=False, default=ItemCategory.OTHER),
    Column("status", SQLEnum(ItemStatus), nullable=False, default=ItemStatus.DRAFT),
    ...
    Column("owner_id", UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, index=True),
...
item_comments = Table(
    ...
    metadata,
    Column("id", UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
    Column("item_id", UUID(as_uuid=True), ForeignKey("items.id"), nullable=False, index=True),
    Column("author_id", UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, index=True),
    Column("content", Text, nullable=False),
    Column("is_approved", Boolean, nullable=False, default=True),
    Column("parent_comment_id", UUID(as_uuid=True), ForeignKey("item_comments.id"), nullable=True),
...
item_views = Table(
    ...
    Column("id", UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
    Column("item_id", UUID(as_uuid=True), ForeignKey("items.id"), nullable=False, index=True),
    Column("viewer_id", UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True),  # Nullable for anonymous views
    Column("ip_address", String(45), nullable=True),
    Column("user_agent", String(500), nullable=True),
    Column("viewed_at", DateTime, nullable=False, server_default=func.now(), index=True),
...
item_permissions = Table(
    ...
    Column("role_id", UUID(as_uuid=True), ForeignKey("roles.id"), nullable=True, index=True),
