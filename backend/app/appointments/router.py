...
match the appointments domain. Replace appointmentservice with an AppointmentService
...
from app.appointments.service import appointmentservice, ItemCommentService
...
async def create_item(
    ...
    **Business Rules**:
    - Only verified users can create appointments
    - New users cannot directly publish appointments (auto-changed to draft)
    - appointments are private by default
    """

    item = await appointmentservice.create_item(db, item_data, current_user)
    # Ensure decision defaults to pending and notify hosts/secretaries via SSE
    try:
        from app.appointments.schemas import ItemUpdate

        meta = item.get("metadata") or {}
        if meta.get("decision") is None:
            meta = {**meta, "decision": "pending"}
            item = await appointmentservice.update_item(
                db, item["id"], ItemUpdate(metadata=meta), current_user
            )
...
async def list_appointments(
    ...
    **Permissions**:
    - Anonymous users: Only public appointments
    - Authenticated users: Public appointments + their own appointments
    ...
    **Sorting Options**:
    ...
    appointments_list, metadata = await appointmentservice.get_item_list(
        db, filters, pagination, current_user
    )

    return ItemListResponse(
        appointments=[ItemResponse(**item) for item in appointments_list], **metadata
    )
...
async def get_item(
    ...
    **Authentication Required**: No (but affects visibility)
    **Permissions**:
    - Public appointments: Anyone can view
    - Private appointments: Owner only (or users with explicit read permission)
...
async def update_item(
    ...
    **Business Rules**:
    ...
    updated_item = await appointmentservice.update_item(
...
async def approve_appointment(
    ...
    updated = await appointmentservice.update_item(
        db, item["id"], ItemUpdate(metadata=new_meta), current_user
    )
    ...
    try:
        ...
        notify_user(
            ...
            f"Appointment {item.get('id')} was approved.",
...
async def deny_appointment(
    ...
    try:
        ...
        notify_user(
            owner,
            "Appointment denied",
            f"Appointment {item.get('id')} was denied.",
            {"appointment_id": str(item.get("id"))},
        )
    except Exception:
        pass
...
async def postpone_appointment(
    ...
    try:
        ...
        notify_user(
            ...
            f"Appointment {item.get('id')} was postponed.",
            {"appointment_id": str(item.get("id")), "until": until},
        )
    except Exception:
        pass
    return ItemResponse(**updated)
...
async def delete_item(
    ...
    **Business Rules**:
    - Published appointments with comments require admin approval
    - Soft delete by default (hard delete for admins only)
    - Deleted appointments are hidden from normal queries
    """
    ...
    success = await appointmentservice.delete_item(
...
async def cancel_appointment(
    ...
    try:
        ...
        notify_user(
            ...
            f"Appointment {item.get('id')} was cancelled.",
            {"appointment_id": str(item.get("id"))},
        )
    except Exception:
        pass
...
async def toggle_appointment_favorite(
    ...
    **Authentication Required**: Yes
    **Permissions**: Any authenticated user (for accessible appointments)

    **Returns**: Updated favorite status and count
    """

    result = await appointmentservice.toggle_favorite(db, item_id, current_user)
    return result
...
async def _list_and_filter(
    ...
    appointments, meta = await appointmentservice.get_item_list(
        db, filters, pagination, current_user
    )
    ...
    def _keep(x: dict) -> bool:
        ...
        if today_only:
            ...
            dt_str = x.get("created_at") or ""
            dt_str = str(dt_str)
            if not _is_today(dt_str):
                return False
        return True

    return [ItemResponse(**x) for x in appointments if _keep(x)], meta
...
async def dashboard_today_approved(
    ...
    appointments, meta = await _list_and_filter(
        db, current_user, decision="approved", today_only=True
    )
    return ItemListResponse(appointments=appointments, **meta)
...
async def dashboard_today_all(
    ...
    appointments, meta = await _list_and_filter(
        db, current_user, decision=None, today_only=True
    )
    return ItemListResponse(appointments=appointments, **meta)
...
async def list_approved(
    ...
    appointments, meta = await _list_and_filter(db, current_user, decision="approved")
    return ItemListResponse(appointments=appointments, **meta)
...
async def list_rejected(
    ...
    appointments, meta = await _list_and_filter(db, current_user, decision="denied")
    return ItemListResponse(appointments=appointments, **meta)
...
async def list_cancelled(
    ...
    appointments, meta = await _list_and_filter(db, current_user, decision="cancelled")
    return ItemListResponse(appointments=appointments, **meta)
...
async def list_requested(
    ...
    appointments, meta = await _list_and_filter(db, current_user, decision="pending")
    return ItemListResponse(appointments=appointments, **meta)
...
async def search_appointments(
    ...
    return ItemListResponse(
        appointments=[ItemResponse(**x) for x in appointments], **meta
    )
...
async def filter_by_date(
    ...
    end_dt = to_dt(end)
    appointments, meta = await appointmentservice.get_item_list(
        db, {}, {"page": 1, "size": 500}, current_user
    )

    def keep(x: dict) -> bool:
        dt_str = str(x.get("created_at") or "")
        try:
            created = datetime.fromisoformat(dt_str.replace("Z", "+00:00")).astimezone(
                timezone.utc
            )
        except Exception:
            return False
    ...
    subset = [ItemResponse(**x) for x in appointments if keep(x)]
    return ItemListResponse(appointments=subset, **meta)
...
async def purge_cancelled(
    ...
    appointments, _ = await appointmentservice.get_item_list(
        db, {}, {"page": 1, "size": 1000}, current_user
    )
    deleted, skipped = 0, 0
    for x in appointments:
        ...
        try:
            ts_dt = datetime.fromisoformat(str(ts).replace("Z", "+00:00")).astimezone(
                timezone.utc
            )
        except Exception:
            skipped += 1
            continue
        if ts_dt <= cutoff:
            ok = await appointmentservice.delete_item(
                db, x["id"], current_user, soft_delete=False
            )
            if ok:
                deleted += 1
            else:
    ...
    return BulkOperationResult(
        total=deleted + skipped, success=True, processed=deleted, failed=skipped
    )
...
async def bulk_appointment_operation(
    ...
    **Returns**: Summary of successful and failed operations
    ...
    result = await appointmentservice.bulk_operation(db, operation_data, current_user)
    return result
...
async def get_my_favorites(
    ...
    **Permissions**: User's own favorites only
    ...
    from app.appointments.models import item_favorites
...
async def admin_get_all_appointments(
    ...
    **Features**:
    - Access to all appointments regardless of visibility
    - Option to include deleted appointments
...
async def admin_restore_item(
    ...
    **Permissions**: Admin only
    ...
    restored_item = await appointmentservice.get_item_list(db, {}, {}, admin_user)
...
async def get_item_analytics(
    ...
    **Analytics Include**:
    ...
    if item["owner_id"] != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only view analytics for your own appointments",
        )
    ...
    total_views_query = (
        ...
        .where(item_views.c.item_id == item["id"])
    )
    total_views = await db.fetch_val(total_views_query) or 0

    unique_viewers_query = (
        select(func.count(distinct(item_views.c.viewer_id)))
        .select_from(item_views)
        .where(
            and_(
                item_views.c.item_id == item["id"], item_views.c.viewer_id.is_not(None)
            )
        )
    )
